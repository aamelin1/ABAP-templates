...
- Картинки статей из вики (C) были GitHub-вложениями (`…/assets/…`) — эти URL **мертвы (404)**, скачать нельзя. Локализовано **38** из офлайна (Notion + старые How-To) по 7 темам; **64** слота помечены `🖼️ скриншот отсутствует` — оригиналы нужно добавить вручную (см. список ниже). Затронутые темы с пробелами: Корреспонденция счетов, FAGL_FCV, fagll03h/PIVB, CDS/IDA, Пользователи/роли, Транспортные запросы, Справочник, частично ALV PIVB и Переключение языка.
